        ...
        <div class="container-fluid">
            <div class="nk-content-inner">
                <div class="nk-content-body">
                    <div class="nk-block-head nk-block-head-sm">
                        <div class="nk-block-between">
                            <div class="nk-block-head-content">
                                <h3 class="nk-block-title page-title">{{__('add')}} {{__('hub')}}</h3>
                    ...
                    <form action="{{ route('admin.hub.store')}}" class="form-validate" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="card">
                            <div class="row">
                                <div class="col-md-12">
                                        ...
                                        <div class="row g-gs">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label class="form-label" for="incharge">{{__('incharge')}} *</label>
                                                    <div class="form-control-wrap">
                                                        <select class="form-select form-control form-control-lg" id="incharge" name="incharge" required>
                                                            <option value="">{{ __('select_incharge') }}</option>
                                                            @foreach($users as $user)
                                                                <option value="{{ $user->id }}" {{ old('incharge') ? ($user->id == old('incharge') ? 'selected' : '') : '' }}>{{ $user->first_name.' '.$user->last_name }}( {{$user->email}})</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
