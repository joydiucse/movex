...
class ParcelController extends Controller
{
    ...
    public function parcelDelete(Request $request)
    {
        ...
        if($stsCheckResponse === true):
            ...
            return back()->with('danger', __($stsCheckResponse));
        endif;
    }

    public function assignPickupMan(Request $request)
    {
        $stsCheckResponse = $this->statusCheck($request->id, "pickup-assigned");
        if($stsCheckResponse === true):
            $parcel = $this->parcels->get($request->id);
            if(hasPermission('read_all_parcel') || $parcel->hub_id == Sentinel::getUser()->hub_id || $parcel->pickup_hub_id == ''
                ||$parcel->pickup_hub_id == Sentinel::getUser()->hub_id || $parcel->transfer_to_hub_id == Sentinel::getUser()->hub_id):
    ...
    {
        $stsCheckResponse = $this->statusCheck($request->id, "delivery-assigned");
        if($stsCheckResponse === true):
            $parcel = $this->parcels->get($request->id);
            if(hasPermission('read_all_parcel') || $parcel->hub_id == Sentinel::getUser()->hub_id || $parcel->pickup_hub_id == ''
                ||$parcel->pickup_hub_id == Sentinel::getUser()->hub_id || $parcel->transfer_to_hub_id == Sentinel::getUser()->hub_id):
    ...
    {
        $stsCheckResponse = $this->statusCheck($request->id, "re-schedule-pickup");
        if($stsCheckResponse === true):
    ...
    {
        $stsCheckResponse = $this->statusCheck($request->id, "re-schedule-delivery");
        if($stsCheckResponse === true):
    ...
    {
        $stsCheckResponse = $this->statusCheck($request->id, "return-assigned-to-merchant");
        if($stsCheckResponse === true):
    ...
    {
        $stsCheckResponse = $this->statusCheck($request->id, "received-by-pickup-man");
        if($stsCheckResponse === true):
            $parcel = $this->parcels->get($request->id);
            if(hasPermission('read_all_parcel') || $parcel->hub_id == Sentinel::getUser()->hub_id || $parcel->pickup_hub_id == ''
                || $parcel->pickup_hub_id == Sentinel::getUser()->hub_id || $parcel->transfer_to_hub_id == Sentinel::getUser()->hub_id):
    ...
    public function partialDelivery(PartialDeliveryRequest $request)
    {
        ...
        $stsCheckResponse = $this->statusCheck($request->id, "partially-delivered");
        if($stsCheckResponse === true):
            $parcel = $this->parcels->get($request->id);
            if(hasPermission('read_all_parcel') || $parcel->hub_id == Sentinel::getUser()->hub_id || $parcel->pickup_hub_id == ''
                || $parcel->pickup_hub_id == Sentinel::getUser()->hub_id || $parcel->transfer_to_hub_id == Sentinel::getUser()->hub_id):
    ...
    {
        // if($this->statusCheck($request->id, "returned-to-greenx")):
            $parcel = $this->parcels->get($request->id);
            if(hasPermission('read_all_parcel') || $parcel->hub_id == Sentinel::getUser()->hub_id || $parcel->pickup_hub_id == ''
                || $parcel->pickup_hub_id == Sentinel::getUser()->hub_id || $parcel->transfer_to_hub_id == Sentinel::getUser()->hub_id):
        ...
        // else:
        //     return back()->with('danger', __('sorry_this_parcel_already_assign_or_delivered'));
        // endif;
    }
    ...
    public function returnToMerchant(Request $request)
    {
        ...
        $parcel = $this->parcels->get($request->id);
        $stsCheckResponse = $this->statusCheck($request->id, "returned-to-merchant");
        if($stsCheckResponse === true):
    ...
    {
        $stsCheckResponse = $this->statusCheck($request->id, "transferred-to-hub");
        if($stsCheckResponse === true):
            $parcel = $this->parcels->get($request->id);
            if(hasPermission('read_all_parcel') || $parcel->hub_id == Sentinel::getUser()->hub_id || $parcel->pickup_hub_id == ''
                || $parcel->pickup_hub_id == Sentinel::getUser()->hub_id || $parcel->transfer_to_hub_id == Sentinel::getUser()->hub_id):
    ...
    {
        $stsCheckResponse = $this->statusCheck($request->id, "transferred-received-by-hub");
        if($stsCheckResponse === true):
            $parcel = $this->parcels->get($request->id);
            if(hasPermission('read_all_parcel') || $parcel->hub_id == Sentinel::getUser()->hub_id || $parcel->pickup_hub_id == ''
                || $parcel->pickup_hub_id == Sentinel::getUser()->hub_id || $parcel->transfer_to_hub_id == Sentinel::getUser()->hub_id):
    ...
    public function deliveryReverse(Request $request)
    {
        ...
        if(hasPermission('read_all_parcel') || $parcel->hub_id == Sentinel::getUser()->hub_id || $parcel->pickup_hub_id == ''
            ...
            if ($parcel->status == $request->status):
                return back()->with('danger', __('this_parcel_has_already_reversed'));
            endif;
        ...
        if($request->has('key')){
            if ($request->key != "") {
                ...
                $query->where('parcel_no','LIKE', "%{$searchKey}%")
                    ...
                    ->orWhereIn('merchant_id', function ($subQuery) use($searchKey){
                        $subQuery->select('id')
                            ->from(with(new Merchant())->getTable())
                            ->where('company',  'LIKE', "%{$searchKey}%");
                    });
