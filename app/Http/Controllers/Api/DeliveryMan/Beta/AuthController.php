...
namespace App\Http\Controllers\Api\DeliveryMan\Beta;
...
class AuthController extends Controller
{
    ...
    public function loginOtp(Request $request)
    {
        ...
        try {
            ...
            if($this->passwordReset($user, $password)):
                ...
                if($this->smsSender('login-otp',$user->phone_number, $sms_body, true)):
                    DB::commit();
    ...
    public function getProfile($user)
    {
        ...
        if (isset($user->image)):
            $data['image'] = asset($user->image->image_small_two);
        else:
            $data['image'] = '';
        endif;
    ...
    //v10 updates
    public function paymentLogsV10(Request $request){
        try {
            if (!$user = JWTAuth::parseToken()->authenticate()) {
                return $this->responseWithError(__('unauthorized_user'), '' , 404);
            }
    ...
    public function formatLogsV10($logs){
        foreach ($logs as $log):
            $log['id'] = $log->id;
            $log['source'] = __($log->source);
            $log['details'] = __($log->details);
