...
class WithdrawController extends Controller
{
    ...
    public function getMerchantInfo(Request $request)
    {
        ...
        foreach ($payable_parcels as $parcel):
            $parcels.= "<input type='text' value=\"$parcel->id\" name=\"parcels[]\" hidden>";
        endforeach;

        foreach ($payable_merchant_accounts as $merchant_account):
            $merchant_accounts.= "<input type='text' value=\"$merchant_account->id\" name=\"merchant_accounts[]\" hidden>";
        endforeach;

        $options = '';
        foreach($payment_account as $method){
            $options .= "<option value=\"$method\">".__($method)."</option>";
        }
...
//        endif;
    ...
    public function filterByMerchantName(Request $request)
    {
        ...
        $withdraws = $query->latest()->paginate(\Config::get('greenx.paginate'));
    ...
    public function filter(Request $request)
    {
        ...
        if ($request->withdraw_id != ""):
            ...
            $withdraws = $query->paginate(\Config::get('greenx.paginate'));
