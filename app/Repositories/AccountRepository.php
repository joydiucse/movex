...
class AccountRepository implements AccountInterface {
    ...
    public function update($request)
    {
        ...
        try{
            ...
            if($this->IncomeLog($delivery_account->id, "delivery_account_update", $delivery_account)):
    ...
    {    
        ...
        if($this->IncomeLog($company_account->id, "company_account_delete", $company_account)):
            DB::beginTransaction();
            try{
    ...
    public function incomeExpenseManage($id, $status, $purpose = '')
    {
        ...
        if($status == 'delivered' || $status == 'partially-delivered'):
            ...
            $merchant_account->details      = 'parcel_totol_delivery_charge_to_greenx';
            $merchant_account->save();
...
//            }
         ...
         ($status == 'partially-delivered' || $status == 'returned-to-greenx' || $status == 'return-assigned-to-merchant')){
...
//            }
         ...
         ($status == 'received' || $status == 'transferred-received-by-hub' || $status == 'transferred-to-hub'
             || $status == 'delivery-assigned' || $status == 'returned-to-greenx' || $status == 'return-assigned-to-merchant')){
             //returned-to-merchant all reversed
             //dd('7');
...
//            }
        ...
        }elseif(($current_status == "return-assigned-to-merchant" || $current_status == "returned-to-greenx" || $current_status == "partially-delivered")
...
//            }
            $this->deliveryTransactionReverse($id);
            $this->receivedTransactionReverse($id);
        }elseif(($current_status == "return-assigned-to-merchant" || $current_status == "returned-to-greenx") &&
        ($status == 'pending' || $status == 'pickup-assigned' || $status == 'received-by-pickup-man')){
            //received reversed
...
//            }
    ...
    public function deliveryTransactionReverse($id){
        ...
        $merchant_account->details      = 'parcel_totol_delivery_charge_to_greenx_reversed';
        $merchant_account->save();
    ...
    public function returnTransactionReverse($id){
        ...
        $merchant_account->amount    = $return_charge;
        $merchant_account->type      = 'income'; // previous expense
        $merchant_account->details   = 'parcel_return_fee_to_greenx_as_parcel_returned_reversed';
        $merchant_account->save();
    ...
    public function incomeExpenseManageCancel($id, $status){
        ...
        if($status == 'cancel'){
            ...
            || $parcel->status == 're-schedule-delivery' || $parcel->status == 'returned-to-greenx'
            ...
            || $parcel->status_before_cancel == 're-schedule-delivery' || $parcel->status_before_cancel == 'returned-to-greenx'
